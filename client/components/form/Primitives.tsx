 ...
 */
/**
 * 2.4.40 -- extracted out of PaymentMethodPicker so a form that only needs
 * "which card" (not the full cash/card/other choice) can get the same
 * inline "+ New card" affordance instead of a 4th hand-rolled copy. Two
 * forms had this exact gap: "Track a new balance" showed a static "No
 * saved cards yet" message with no action, and the edit-transaction form's
 * card picker had no `else` branch at all when the list was empty.
 */
export function CardPicker({
  cardId, onCardIdChange, cards, onSaveCard,
}: {
  cardId: string | null;
      ...
      {!showAddCard ? (
        ...
        <div className="rounded-xl p-3 space-y-2" style={{ background: T.ink, border: `1px solid ${T.line}` }}>
          ...
          <div className="grid grid-cols-2 gap-2">
            ...
            <div>
              <Label htmlFor="card-picker-new-last4">Last 4 digits</Label>
              <FocusInput
                id="card-picker-new-last4"
                type="text"
                inputMode="numeric"
                maxLength={4}
                value={newCardLast4}
...
export function PaymentMethodPicker({
  ...
  return (
    <div>
      ...
      <div className="grid grid-cols-3 gap-2 mb-2">
        {PM_OPTIONS.map((p) => (
          <button
            key={p.value}
            type="button"
            onClick={() => { onChange(p.value); onCardIdChange(null); }}
      ...
      {value === "card" && (
        <CardPicker cardId={cardId} onCardIdChange={onCardIdChange} cards={cards} onSaveCard={onSaveCard} />
      )}
    </div>
  );
}
