...
18. **A stated direction gets re-derived from the math, not carried forward as settled.** Rule 7 covers a signed *field* before it ships; this covers a directional *claim* in a plan, a report, or a triage note — which way a value flips, which side of a transition negates, whether a conversion inverts. Any such claim is re-derived from the actual formula before anything is built on it, never inherited from an earlier summary as already-established. Four sign errors on record: `debtAdjustment`, `TRANSFER`'s original amount polarity, the retag-to-TRANSFER direction, and a triage note asserting that INCOME→TRANSFER should flip the sign when it should not. The first three were caught only after code had been built on them; the fourth was caught *before*, and only because it was re-checked against the formula instead of trusted from the prior write-up. A direction is a one-bit claim that is cheap to verify and expensive to inherit wrong — the cost of checking never approaches the cost of building on it backwards.
