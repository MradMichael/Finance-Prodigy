...
8. **A phase or finding's status is updated in the same commit that changes it, not in a later doc pass.** Phase 2.7 merged in three sub-phases (`feat/sync-merge-subphase1`, `feat/sync-merge-subphase2`, and the autoSync/Profile-Merge wiring folded in after) while `ROADMAP.md` kept reading "not started, cohort-blocking" through all three — found only because a 2026-09-03 consolidation pass fact-checked the doc against the actual repo instead of trusting it. If a commit merges work that changes what a tracked phase or finding's status line says, that status line changes in the same commit. A status update queued for "the next time someone rereads the doc" is a status update that doesn't happen.
9. **Disposable test accounts are removed in the same session that creates them.** `server/.env`'s `DATABASE_URL` points at the same production Neon database the live app uses — there is no separate dev database. Live-verification passes routinely sign up throwaway `@example.invalid` accounts against it; 38 of them, across a dozen sessions since 2026-09-01, were never cleaned up (2.4.62) before this rule existed, found only by accident while a different session's own 6 rows were being removed. Create, verify, delete — every session, not just the ones that happen to check.
10. **Reconcile before reporting.** When work is fanned out to sub-agents, contradictions between their outputs are surfaced explicitly, never resolved silently into a summary. A sub-agent reported "10 tests" descriptively while another reported "11/11 passed" after a live run, for the same investigation; the summary carried the first figure forward without reconciling it against the second, implying a failing test where none existed — the suite was 11/11 green throughout. If two agents report different values for the same thing, both are reported and the discrepancy is named, not averaged away.
11. **Restate scope before building.** Before implementing an approved plan, restate what it authorizes and explicitly name anything it said not to touch. An approved plan for the Balance Check fix stated explicitly that the badge was untouched in every case; the implementation flipped it anyway once a case's numbers fully explained the discrepancy. Reverted on request and logged as its own correction rather than folded silently into the original finding. If implementation would exceed the stated scope, stop and ask before writing the code.
12. **Counts come from files, not memory.** Any status count, item list, or remaining-work enumeration is produced by re-reading the source file at that moment, never reconstructed from earlier in the session. A batch close-out enumerated the remaining open items from recall rather than re-reading the tracking file, lost track mid-list, and happened to land on the correct total anyway — right by coincidence, not by verification. Re-read before reporting a number, every time, even when the number feels remembered correctly.
13. **Verify absence, don't assume it.** NOT STARTED or ABSENT means searched for and confirmed missing — stubs, dead config, orphaned files, schema fields, env keys — never "not found on a first pass." Four items were verdicted CONFIRMED against a claim that something was absent, when nobody had actually searched for it — the absence was assumed, not verified. State which searches were run before a claim of absence stands.
14. **Name the competing hypothesis.** Any diagnosis stated with confidence names at least one alternative explanation and what evidence would distinguish it from the one being asserted; if no plausible alternative exists, that absence is stated explicitly rather than left implicit. A local authentication failure was diagnosed as an old, already-authenticated connection surviving a credential rotation cutover while a fresh attempt failed; the connection cited as evidence had actually been established via a restart after the rotation was already supposed to be complete, so the proposed mechanism didn't hold even though the bottom-line conclusion — the new credentials weren't authenticating — was confirmed correct once tested directly. A right answer reached by the wrong mechanism is a defect, not a pass.
15. **Read the error before inferring from it.** When a tool, service, or API surfaces an explicit error message or annotation, retrieve and read it before reasoning about the failure from surrounding evidence. A CI failure was diagnosed across two separate rounds from indirect signals — empty steps, no runner assigned — while the one job annotation that stated the actual cause outright went unread until a later pass. Indirect evidence is a fallback for when no direct statement of cause exists, not a substitute for checking whether one does.
16. **Flag what the fix doesn't fix.** Any fix applied to a defect that was diagnosed as having a deeper or broader cause states, in the same report, whether that underlying cause is closed or still open. A copy fix stopped the Projections slider from implying that added capacity moves a goal's completion date, but goals remain structurally incapable of completing earlier than their own target date regardless of capacity — a gap the copy fix doesn't touch. That gap was logged only because it was asked for as a separate item, not surfaced unprompted. A fix that narrows a bug's visible symptom without closing its cause says so in the same report that announces the fix.
17. **Own-work findings get logged like any other.** Defects and quirks in Claude Code's own reporting, plans, or process are logged in the audit with the same numbering and evidence standard as code findings. The 10-tests-vs-11/11-passed conflation (Rule 10's case) was, once actually investigated, correctly diagnosed as Claude's own reporting error rather than a code defect — but that diagnosis was delivered only in conversation and never written into the audit doc as its own finding. Rule 10 is about not making that conflation in the first place; this rule is about logging it, with a number and evidence, once it has been made and diagnosed. A correct diagnosis that only exists in the transcript is unavailable to the next session; it gets a finding number like anything else.
18. **A claim inherited from an earlier write-up gets re-derived before anything is built on it.** Rule 7 covers a signed *field* before it ships; this covers a *claim* — in a plan, a report, or a triage note — and the failure mode is one while the shapes are two. **Tell one, a DIRECTIONAL claim:** which way a value flips, which side of a transition negates, whether a conversion inverts. Re-derive it from the actual formula, never from an earlier summary. Four sign errors on record: `debtAdjustment`, `TRANSFER`'s original amount polarity, the retag-to-TRANSFER direction, and a triage note asserting that INCOME→TRANSFER should flip the sign when it should not. Three were caught only after code had been built on them; the fourth was caught before, and only because it was re-checked against the formula instead of trusted from the prior write-up. **Tell two, a SCOPE or LOCATION claim:** a finding named after the route it surfaced by rather than the code that is wrong. The tell is a title whose subject is something the *user* does — import a file, run a test, open a screen — rather than something the *code* does. Its mechanical form, and the reason this tell is checkable rather than a matter of judgement: **before planning, grep the misbehaving function's callers and state the number.** 2.4.106 read as an import bug and was a heal-effect bug reachable from nine `saveData` callers; 2.4.99 read as a test timeout and was a ~350ms main-thread freeze on three production screens. In both, the caller count is what falsified the title. **What the evidence shows, recorded here so the rule does not overstate itself:** this rule has never been in force. It sat on an unmerged branch from 2026-09-09 while the audit cited it, so no session ever read it in this document. It describes a habit that was already being practised rather than one it created — Phase 5a handled the sharpest directional question in this codebase (the emergency-fund and debt deltas carry opposite signs by construction) correctly, tested by round trip rather than by asserting the sign, written by a session that had never read this rule. Treat it as a description of what care looks like here, not as evidence that writing a rule down produces it; 2.4.115's own conclusion is that knowing a pattern did not prevent its next instance.
