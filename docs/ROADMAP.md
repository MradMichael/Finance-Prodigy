...
| 1.5 | Display and reporting in either currency, every total labelled. **Built — 2026-08-21** (`f80fa48` on `feat/currency-display-reporting`, off `main`). A full sweep found exactly 5 gaps, all in the two files 1.4 deliberately left untouched (`printReport.ts`, `FinancialDashboard.tsx`); both now use `fmtCur`/USD-converted display consistently with every other screen. `tsc` clean, 279/279 tests pass, both builds clean, live-verified in a local dev build (LBP goal + debt: correct `L£` on Overview, correct converted USD in both PDF reports). **Branch only, held for review — not merged.** Owner confirming 1.4 on the deployed build first; both 1.4's ✅ and 1.5's merge happen together. | Yes — feature complete |
