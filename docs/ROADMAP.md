...
**Status:** in progress (updated 2026-08-26) · **Depends on:** Phase 2.5 · **Blocks:** Phase 2 (cohort launch) — recommended, see below. **2.6.1 (data model + migration) merged, live. 2.6.2 (`derivedEfBalance`/`derivedDebtBalance`, pure logic, shipped unwired) merged, live, still unwired. 2.6.3(a) (the flip) approved, held for review. 2.6.3(b) (soft delete) built, live-verified, held for review. 2.6.3(c) (edit-transaction UI, EF checkboxes/`recordDebtPayment`/`SetupScreen.tsx`'s EF field become transaction-creating) built, live-verified end-to-end, branch `fix/edit-transaction-2.6.3c` held for review — all three sub-phases now complete, held for a combined merge/deploy per the owner's decision.**
>
