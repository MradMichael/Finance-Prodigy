...
**Status:** sub-phase 1 merged to `main` (`16a935a` via `86b55af`), Vercel `Ready` in production. Sub-phase 2 built, held for review, not yet merged — Rule 6. Design approved 2026-08-26 · **Depends on:** Phase 2.6 (all of 2.6.3, including (c)) — met, 2.6.3(c) merged and live · **Blocks:** Phase 2 (cohort launch).
...
1. **`mergeTransactions(local, server)`, pure engine, tests-first, unwired.** Union by id, tombstone-rank rule (`purgedAt` > `deletedAt` > active, unconditional), `updatedAt`-based last-writer-wins for genuine same-id conflicts, order-independent per-id outcome. **Built 2026-09-01 on `feat/sync-merge-subphase1` (`16a935a`). Merged to `main` (`86b55af`) 2026-09-01, Vercel `Ready` in production.**
2. **`syncService.ts` wiring (`mergeAndPush`)** — pull-merge-push flow on a conflict, using sub-phase 1's engine. No server change (confirmed: the server stores one opaque blob, zero merge awareness). **Scope decision: non-transaction fields (goals/debts/recurring/assets/cards/trackedBalances/settings) are NOT merged this phase — kept as local's own values (today's pick-a-side, local wins), transaction merge layered on top. A genuine divergence in those fields is silently resolved "local wins," same as before this phase — not detected or surfaced. Flagged in the function's own doc comment, not silently assumed solved.** **Trace requirement (owner's instruction, 2026-09-01): a silent last-writer-wins on a transaction's amount should leave a trace, even though it resolves correctly.** `mergeTransactions`'s `conflictsResolved` (a bare count) was extended, additively, with `conflicts: StoredTransaction[]` — the winning copy of each resolved conflict, so a caller can name what changed (amount, description, date), not just that something did. `mergeAndPush` surfaces both through to its own return value. **Built 2026-09-01 on `feat/sync-merge-subphase2` (`371eb32`), tsc clean, 504/504 full suite (7 new), client build clean. Held for review, not merged — Rule 6.**
3. **`autoSync`'s background merge-on-409 + toast** (`page.tsx`), and the Profile page's new primary "Merge" action alongside the existing Push/Pull. **Owns the actual user-facing wording** — sub-phase 2 only guarantees the data (`conflicts`) is there to write it with; proposed toast copy (not yet built): 0 conflicts → "Merged with your other device — N new transactions added." ≥1 conflict → names it, doesn't just count it — e.g. "Merged with your other device — 1 edit conflict resolved (kept the newer edit to 'Groceries, Aug 3' — $75, was $50)." for a single conflict; for multiple, a count plus a way to see which ones (not yet decided: inline list vs. link to Transactions).
...
- A forward view shows monthly disposable capacity, **including the step change when an obligation terminates.** ✅ **Step change: 2026-09-13** (see below). 🔴 **Monthly disposable capacity figure: still not started** — the shipped strip reports what frees up and when, at base capacity 0; it does not render a per-month disposable-capacity figure, because what that figure *is* remains undecided (`month.netCashFlow` and `budgetTargets.savings` are different numbers, and Projections seeds its planning amount from the latter).

**Acceptance:** given the owner's real installment schedule, the forward view correctly shows capacity increasing at termination. **Met for the step change, 2026-09-13; the phase stays open.**

**2026-09-13 — bounded recurring wired into Projections** (`feat/bounded-recurring-capacity`). `capacityByMonth` had been built and tested since 2026-09-01 and called from nowhere; wiring it in unchanged would have produced a permanently flat line for the owner's real data, because it filtered on `endDate !== null` and the only bounded obligation in that data (Uni, $750/mo capped at $6,750) has `endDate: null` and a `totalAmount` cap. The one bound type it handled was the one not in use — and that blindness was asserted as intended behaviour by a shipped single-fixture test, now rewritten around a fixture PAIR (both `endDate: null`, opposite outcomes), since one fixture structurally cannot discriminate here.

New resolver `capacityFreedFrom(r, transactions, asOf)` answers "from what date does this stop consuming capacity", which is a different date per bound type: `endDate` already means "terminates on this date" and passes through unchanged (every prior `capacityByMonth` expectation preserved), while `totalAmount` resolves via `remainingInstallments` to `endsOn` — the *last owed* payment, still due — so freed-from is the cycle that would have followed. Conflating them puts Uni's step in April 2027, a month in which $750 is genuinely owed. `RecurringCapacityInput.endDate` renamed to `freedFromDate`, because callers building the old field from `StoredRecurring.endDate` alone *was* the defect. Acceptance verified against the owner's real Uni item, asserting the step lands May 2027 and **not** April, with a regression lock proving the old `endDate`-only path reports nothing at all. 670/670 tests, tsc and build clean.

**What this deliberately does NOT do:** the plan below the strip is still computed on one flat monthly amount for every month and does not consume the freed capacity — the strip's own copy says so outright rather than implying otherwise, since two irreconcilable figures on one screen is a failure mode this project has already logged once. Making the projection engine consume a capacity *schedule* (`projectCompletion` is closed-form `remaining / rate`; `runSimulation` resets `budget` to a constant each month; `fastestGoalCompletion`'s O(1) shortcut is load-bearing on flat capacity) is a separate, much larger change, gated on an open product decision: **when a bounded obligation ends, does the plan spend the freed money, or keep spending what the user dialed and merely report that more became available?** A pre-check on 2026-09-13 confirmed every currently projected date (EF 2030-09, debt 2031-11, South Korea 2036-06) falls years past Uni's 2027-05 step, so the change would not be cosmetic — but the decision is the owner's, not a technical one.
...
**Status:** COMPLETE 2026-09-01 — all four sub-phases built and merged to `main`, Vercel `Ready` in production. 2.4.44 and 2.4.50 closed (`docs/AUDIT_2026-08.md`). Owner's real-account spot-check (their own 1-active/3-paused goals, unpausing a second) still outstanding, planned for their next laptop session — doesn't block this phase's own completion, since it verifies the account, not the mechanism (already fixture- and test-verified). · **Depended on:** Phase 1, 2.4.44 — no longer blocked on Phase 3 in full (path B, see Phase 3's own 2026-09-01 note; Phase 3 itself stays open with its own acceptance criteria, only partially satisfied by F3's internal capacity math)
...
2. **Capacity input including the step-change** — internal Phase-3-slice per path B above (`capacityByMonth`, UTC-anchored month-stepping off `StoredRecurring.endDate`). **Built 2026-09-01 on `feat/goal-feasibility-engine-subphase2` (`7d0aea8`). Merged to `main` 2026-09-01. Wired 2026-09-13** — first consumer is Projections' "Looking ahead" strip (`feat/bounded-recurring-capacity`), which also had to widen the input from `endDate` to a resolved `freedFromDate` before it reported anything for the owner's real data. Phase 3's fuller forward-view UI (a per-month disposable-capacity figure) remains the open item.
...
**Status:** APPROVED 2026-09-01, before the cohort — design below, not yet built. Sub-phases with SAFE STOP markers, tests-first, same discipline as Phase 1/2.6 · **Depends on:** Phase 1 · **A THIRD, DISTINCT tier — not "small" or "large."** Originally split into two versions 2026-09-01 (small: USD-plus-one, locked at setup; large: any currency pair, no privileged base); a real cohort case (a tester in Egypt needing EGP and SAR *simultaneously*) proved the small version has zero value for her, and reassessment found the large version's own "any pair, no privileged base" framing overstates what's actually needed. **Phase 9-N**, below, is genuinely its own size: N currencies, but still USD-anchored (every currency converts to/from USD; no currency converts directly to another), which is the structural fact that keeps this well short of Phase 9-large.

**The ask:** a user needs more than one non-USD currency in the same account at once (the diaspora case: a Lebanese person in Dubai thinks in AED; a tester with ties to both Egypt and Saudi Arabia needs EGP and SAR simultaneously) — not just a configurable single second currency (Phase 9-small, below, confirmed to not cover this case at all).

### Phase 9-N — N configurable currencies, USD-anchored (APPROVED, this is what gets built)

**Why this is neither small nor large.** `toUSD(amount, currency, lbpRate)` already takes `currency` as an explicit parameter at every one of its ~33 production call sites (confirmed by direct grep, 2026-09-01) — it's never a global/implicit lookup. That's the load-bearing fact: replacing the scalar `lbpRate: number` with a map (`rates: Record<string, number>`) and having `toUSD` index by the currency it's already given is a **uniform, mechanical change** at nearly all 33 sites, not a redesign per site — the same class of touch Phase 9-small's own 170-of-172 finding already established, just applied to a lookup instead of a rename. The architecture stays USD-anchored throughout: N independent USD-rates, never an N×N cross-rate matrix — that's what Phase 9-large's "any pair, no privileged base" framing would actually require, and this doesn't.

**Real logic sites, not just naming (2 today, becomes N-aware, not N-times-larger):** the "amount < 500 in LBP looks like a mistyped USD entry" heuristic (`InputPanel.tsx:212`, `EditTransactionSheet.tsx:95`) is tuned to LBP's ~89,500/USD magnitude. For SAR (~3.75/USD) or EGP (~48/USD), that threshold is nonsense as written — a SAR entry of 500 is a completely ordinary, legitimate amount, not a likely mistype. **Owner's explicit requirement:** this becomes currency-relative (e.g., a threshold scaled by that currency's own rate, not a flat 500), not left as a single hardcoded constant with a new currency silently inheriting LBP's assumption.

**Migration:** cheap, not the expensive part. Wrap the existing scalar `lbpRate` and `lbpRateHistory` array into `{ LBP: oldValue }` / `{ LBP: oldHistory }` — non-clobbering, zero risk to existing USD/LBP data, identical shape to every migration this project has already shipped (2.5.1, 2.6.1, Phase 1.2's own currency migration).

**The genuinely new work is UI, not data:** Setup's rate field is one labeled number input today (`SetupScreen.tsx`) — becomes a repeatable list (pick a currency, enter its rate, add another). `CurrencyScreen.tsx`'s rate-history/devaluation-stress-test view is scoped to one currency today — needs a per-currency section or selector. Neither is a small tweak; both are real screen redesigns.

**Sub-phases, tests-first (money-adjacent), SAFE STOP after each — held for review, none merged without explicit go-ahead:**

1. **Data model + migration.** `Currency` widens from `"USD" | "LBP"` to an open string keyed against a new `currencies: string[]` list on `LocalFinancials` (which currencies this account has ever used, append-only — no removal once a transaction references one, matching every other "can't un-happen" constraint already in this codebase). `lbpRate`/`lbpRateHistory`/`DEFAULT_LBP_RATE` become `rates: Record<string, number>` / `rateHistory: Record<string, {ym,value}[]>`, migration wraps existing values under `"LBP"`. **Nothing reads the new fields yet** — same "invisible sub-phase" discipline as every prior Phase's sub-phase 1. SAFE STOP: trivial, `main` behaviorally identical, richer schema only.
2. **Conversion engine rewiring.** `toUSD`/`toUSDForMonth`/`valueForMonth`-family functions become map-aware (`rates[currency]` instead of a bare scalar). The ~33-site sweep: every call site threads `.rates` instead of `.lbpRate`. Tests-first: the exact multi-currency case written first (an account with USD + EGP + SAR transactions in the same month, each converting correctly against its own rate, none bleeding into another's). Still unwired from any NEW currency being selectable — existing LBP-only accounts must produce byte-identical output before this sub-phase is considered done, proven by re-running the full pre-existing currency test suite unchanged. SAFE STOP: the conversion layer is correct and proven, but nothing new is user-reachable yet.
3. **Mistyped-entry heuristic becomes currency-relative.** Tests-first: LBP at its existing ~500 threshold (regression-locked, must not change for current users), plus a currency with a materially different magnitude (e.g. SAR) proven NOT to false-positive at the same raw number. SAFE STOP: a narrow, self-contained fix, real behavior change but tightly scoped.
4. **Setup UI: add/manage N currencies.** The repeatable currency-picker-plus-rate-entry list, replacing the single LBP field. Each currency, once it has any transaction referencing it, becomes non-removable from the account (append-only `currencies` list, per sub-phase 1). SAFE STOP: real, user-visible, but additive — an LBP-only account sees no behavior change until they deliberately add a second currency.
5. **`CurrencyScreen.tsx`: per-currency rate history and stress test.** Extends the existing single-currency devaluation view to a per-currency selector or stacked sections. SAFE STOP: the last piece, completes the acceptance criteria below.

**Acceptance:** an account can hold transactions in USD plus any number of additional currencies simultaneously, each converting correctly against its own historized rate; a currency, once used, can't be silently reinterpreted as a different one later; the mistyped-entry guard doesn't false-positive on a legitimately-small amount in a low-magnitude currency; an existing USD/LBP account is byte-identical in behavior unless the owner deliberately adds a currency.

**Estimate: 4–6 sessions** (owner-approved at this range, 2026-09-01) — data model/migration (session 1), conversion engine + the 33-site sweep + tests (sessions 2-3), the mistyped-entry fix (partial session), Setup's multi-currency UI (session 4-5), CurrencyScreen's per-currency view (session 5-6). Scheduled **before the cohort launch** — update the Phase 2 precondition list and the sequencing summary below once sub-phase 1 actually starts.

**SAFE STOP** (Phase 9-N, per sub-phase above).
